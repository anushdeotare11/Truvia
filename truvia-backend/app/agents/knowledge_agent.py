...
class KnowledgeAgent:
    def __init__(self):
        ...
        if self.api_key and "your-anthropic-key" not in self.api_key and len(self.api_key) > 10:
            ...
            logger.info("Initialized KnowledgeAgent with Anthropic API client")
        else:
            self.client = None
            logger.warning("No Anthropic API key configured. Running KnowledgeAgent in degraded local mode.")
    ...
    async def _generate_llm_answer(self, query: str, context: str) -> str:
        ...
        try:
            ...
            logger.error(f"Claude RAG failed: {str(e)}")
            raise
